...
void Monster::follow_path(std::vector<Point> path){
...
void Monster::_update(const UpdateState& us)
{
    ...
    if(following_path) {
        ...
        if(pos == Vector2(path[step].x*_game->cell_size.x, path[step].y*_game->cell_size.y)){
            if(step == path.size()-1){
				...
				game->getMainPlayer()->shoot(monsterType);
                this->shoot(lifePoint);
            }else {
                step++;
...
const Vector2 &Monster::getPos() const {
    return pos;
}

void Monster::shoot(int ammoType){
    lifePoint -= ammoType;
    if(lifePoint <= 0){
		_ship->setVisible(false);
		game->getMonsterGarbage().push_back(this);
    }
}
